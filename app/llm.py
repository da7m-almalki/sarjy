...
extract = Agent(
    ...
    instructions=(
        ...
        "The context may include the previous situation, what the assistant just told them, "
        "including any offered alternatives. Use it ONLY to resolve what the user points at: "
        "'the first one', 'the Salem one', 'the earlier one' fill the barber and time of that "
        "option, intent=booking_info. If the reference is ambiguous between options ('that "
        "works' with several on offer), fill nothing. The previous situation is never itself "
        "a source of new fields, and a question still fills nothing. "
